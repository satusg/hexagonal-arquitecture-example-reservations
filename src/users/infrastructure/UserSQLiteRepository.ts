...
export class UserSQLiteRepository implements UserRepository {
  ...
  async delete(userId: UserId): Promise<void> {
    await this.ensureInitialized();
    return new Promise((resolve, reject) => {
      this.db.run('DELETE FROM users WHERE id = ?', [userId.toString()], err => {
        if (err) reject(err);
        else resolve();
      });
    });
  ...
  async findById(userId: UserId): Promise<User | null> {
    await this.ensureInitialized();
    return new Promise((resolve, reject) => {
      this.db.get('SELECT * FROM users WHERE id = ?', [userId.toString()], (err, row: any) => {
        if (err) reject(err);
  ...
  async findByEmail(email: UserEmail): Promise<User | null> {
    await this.ensureInitialized();
    return new Promise((resolve, reject) => {
      this.db.get('SELECT * FROM users WHERE email = ?', [email.toString()], (err, row: any) => {
        if (err) reject(err);
        else if (!row) resolve(null);
        else {
          try {
